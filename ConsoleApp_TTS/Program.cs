...
//var ss = speech.SynthesizeTextToStreamAsync("123");

string text = "烏薩奇是超人氣日本角色《吉伊卡哇》中的熱門角色，外觀為亮黃色兔子。她個性活潑好動、調皮且自由奔放，總是發出「呀哈」、「嗚啦」等獨特叫聲，同時擁有極強的戰鬥力與「除草檢定5級」證照，是充滿反差萌的開心";


System.Speech.Synthesis.SpeechSynthesizer speech = new();
speech.SetOutputToDefaultAudioDevice();
StringBuilder strb = new();
speech.PhonemeReached += (sender, args) =>
{
    strb.Append(args.Phoneme);
    System.Diagnostics.Trace.WriteLine($"Phoneme:{args.Phoneme} Prompt:{args.Prompt}");
};
//speech.SpeakSsml(text);
speech.Speak(text);
File.WriteAllText("123.txt", strb.ToString());
