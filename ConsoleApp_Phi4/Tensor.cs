...
namespace Microsoft.ML.OnnxRuntimeGenAI
{
    ...
    public class Tensor1 : IDisposable
    {
        private IntPtr _tensorHandle;
        private bool _disposed = false;

        public Tensor1(IntPtr data, Int64[] shape, ElementType type)
        {
            Result1.VerifySuccess(NativeMethods.OgaCreateTensorFromBuffer(data, shape, (UIntPtr)shape.Length, type, out _tensorHandle));
        }

        internal Tensor1(IntPtr tensorHandle)
        {
        ...
        {
            Result1.VerifySuccess(NativeMethods.OgaTensorGetType(_tensorHandle, out ElementType element_type));
            return element_type;
        }

        public Int64[] Shape()
        {
            Result1.VerifySuccess(NativeMethods.OgaTensorGetShapeRank(_tensorHandle, out UIntPtr size));
            Int64[] shape = new Int64[size.ToUInt64()];
            Result1.VerifySuccess(NativeMethods.OgaTensorGetShape(_tensorHandle, shape, size));
            return shape;
        }
        ...
        public ReadOnlySpan<T> GetData<T>()
        {
            ...
            Result1.VerifySuccess(NativeMethods.OgaTensorGetData(Handle, out IntPtr data));
            unsafe
            {
    ...
    public class NamedTensors1 : IDisposable
    {
        private IntPtr _namedTensorsHandle;
        private bool _disposed = false;

        internal NamedTensors1(IntPtr namedTensorsHandle)
        {
